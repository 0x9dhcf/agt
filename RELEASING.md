...
runs tests, packages a source tarball, and publishes a GitHub Release with
...
1. The tag name **must** match `PROJECT_VERSION` in `CMakeLists.txt`
   (i.e. tag `v0.3.0` requires `project(agt VERSION 0.3.0 ...)`).
2. `CHANGELOG.md` **must** contain a `## [X.Y.Z]` section matching the tag.
3. The release build and debug build (with sanitizers + ctest) must pass.
...
1. **Bump the version** in `CMakeLists.txt`:
   ```cmake
   project(agt VERSION X.Y.Z LANGUAGES CXX)
   ```
...
2. **Update `CHANGELOG.md`**:
   ...
   - Update the link references at the bottom of the file:
     ```markdown
     [Unreleased]: https://github.com/0x9dhcf/agt/compare/vX.Y.Z...HEAD
     [X.Y.Z]: https://github.com/0x9dhcf/agt/releases/tag/vX.Y.Z
     ```
...
6. **Watch the workflow**:
   ...
   `https://github.com/0x9dhcf/agt/releases/tag/vX.Y.Z` with
   `agt-X.Y.Z.tar.gz` attached.
...
5. Builds the `debug` preset and runs `ctest --preset unit` (sanitizer-enabled).
6. Creates `agt-X.Y.Z.tar.gz` via `git archive` (reproducible, excludes `.git`).
7. Extracts the `## [X.Y.Z]` section from `CHANGELOG.md` into `RELEASE_NOTES.md`.
   Fails if the section is missing.
8. Publishes the GitHub Release via `softprops/action-gh-release@v2`,
   attaching the tarball and using the extracted notes as the body.

## Recovering from a failed release

If the workflow fails (mismatched tag, missing changelog entry, test failure,
