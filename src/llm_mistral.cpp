...
std::string llm_mistral::url() const {
  return "https://api.mistral.ai/v1/chat/completions";
}

std::vector<std::string> llm_mistral::headers() const {
  return {"Content-Type: application/json",
          "Authorization: Bearer " + key_};
}

Json llm_mistral::build_request(const Json &input) const {
  Json out;
  out["model"] = model_;
...
Json llm_mistral::parse_response(const Json &output) const {
...
Json llm_mistral::build_stream_request(const Json &input) const {
  auto out = build_request(input);
  out["stream"] = true;
  return out;
}

std::string llm_mistral::parse_stream_event(const std::string & /*event*/,
