...
//     auto name = call["name"].get<std::string>();
//     auto id = call["id"].get<std::string>();
//     auto input = call["input"].get<std::string>();
//     Json args = Json::parse(input);
//
//     auto it = tool_map.find(name);
//     if (it == tool_map.end()) {
//       pending.push_back(
//           {.id = std::move(id), .name = std::move(name), .error_msg = "tool not found: " +
//           name});
//       continue;
//     }
//
//     // validate synchronously — cheap, schema check
//     try {
//       nlohmann::json_schema::json_validator validator;
//       validator.set_root_schema(it->second->parameters());
//       validator.validate(args);
//     } catch (const std::exception& e) {
//       if (hooks.on_tool_stop)
//         hooks.on_tool_stop(*it->second, args, Json{{"error", e.what()}});
//       pending.push_back({.id = std::move(id),
//                          .name = std::move(name),
//                          .args = std::move(args),
//                          .invalid = true,
//                          .error_msg = "invalid tool input: " + std::string(e.what())});
//       continue;
//     }
//
//     if (hooks.on_tool_start && !hooks.on_tool_start(*it->second, args)) {
//       pending.push_back(
//           {.id = std::move(id), .name = std::move(name), .args = std::move(args), .denied =
//           true});
//       continue;
//     }
//
//     // Phase 1 — launch async
//     auto& tool = it->second;
//     pending.push_back({.id = std::move(id),
//                        .name = std::move(name),
//                        .args = args,
//                        .future = std::async(std::launch::async, [&tool, args, &options]() {
//                          return tool->execute(args, options.context);
//                        })});
//   }
//
...
//     Json result;
//
//     if (!p.error_msg.empty()) {
//       result = p.error_msg; // plain string, no .dump()
//     } else if (p.denied) {
...
//         result = p.future.get();
//       } catch (const std::exception& e) {
//         result = "tool execution failed: " + std::string(e.what());
//       }
//
//       // find tool for hook — only valid futures had a tool
//       auto it = tool_map.find(p.name);
//       if (it != tool_map.end() && hooks.on_tool_stop)
//         hooks.on_tool_stop(*it->second, p.args, result);
//     }
//
//     messages.push_back(
//         {{"role", "tool"},
//          {"call_id", p.id},
//          {"content", result.is_string() ? result.get<std::string>() : result.dump()}});
//   }
// }
...
//
//   futures.reserve(resp["call"].size());
...
//         validator.validate(args);
//       } catch (const std::exception& e) {
//         result = "invalid tool input: " + std::string(e.what());
//         messages.push_back({{"role", "tool"}, {"call_id", id}, {"content", result.dump()}});
//         continue;
//       }
//       if (hooks.on_tool_start && !hooks.on_tool_start(*it->second, args)) {
//         result = "tool call denied";
//       } else {
//         result = it->second->execute(args, options.context);
//       }
//       if (hooks.on_tool_stop)
//         hooks.on_tool_stop(*it->second, args, result);
//     } else {
